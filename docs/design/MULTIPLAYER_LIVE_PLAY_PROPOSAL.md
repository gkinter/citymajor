...
- **Cheating Concerns:** Low. Since it is cooperative and asynchronous, cheating only affects the shared city. Basic server-side validation of save files (checksums) prevents blatant corruption, but strict anti-cheat is unnecessary.
...
- **Sync Model:** Hybrid snapshot sync + input forwarding. Client-authoritative with a host-canonical clock. The host runs the main simulation loop and broadcasts state snapshots, while clients forward their inputs (e.g., zoning commands, road placement) to the host. Includes desync detection via CRC32 subsystem checksums.
